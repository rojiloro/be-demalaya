...
const createPersonalInformation = async (req, res) => {
    try {
        ...
        if (!fullname) {
            return res.status(400).send("field fullname is required!");
        }
        
        if (!email) {
            return res.status(400).send("field email is required!");
        }

        if (!phonenumber) {
            return res.status(400).send("field phone number is required!");
        }

        if (!preferedcontactmethod) {
            return res.status(400).send("field prefered contact method is required!");
        }
